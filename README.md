...
```json
{
  "mcpServers": {
    "modast": {
      "command": "node",
      "args": ["E:/github/ModAST-MCP/dist/index.js"]
    }
  }
}
```
...
`mode` accepts `auto`, `cpp`, or `modules` and defaults to `auto`. Auto mode checks the compilation database for `.cppm`, `.ixx`, or `.mpp` units. Pure `cpp` mode skips PCM/modmap discovery and never enables clangd's experimental module support.
...
`format` delegates to clangd/clang-format and honors the project's `.clang-format`. It is preview-only by default and returns the formatted text plus LSP edits. `apply=true` is required to write the source; an applied edit is immediately synchronized back to the persistent clangd document.

`module_quality` reports long function bodies in module interface units. The rule intentionally ignores implementation units and short inline/constexpr accessors. It is a warning-level architecture check: exported declarations stay in `.cppm`, while substantial business logic should normally live in the matching `.cpp` module implementation unit.
